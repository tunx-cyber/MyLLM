...
在制作数据集过程中，有两种做法：一种是右移一位，一种是不右移。  
在预训练的过程中，是需要右移一位的，目的是预测下一个token  
在SFT中，在 SFT 阶段，模型的输入是 [指令 + 输入]，输出是 [期望的回答]。目标：让模型生成的文本与人工标注的标准答案（即标签）完全一致。label是input id的克隆，并且mask掉prompt的部分。也可以分段合成label。
